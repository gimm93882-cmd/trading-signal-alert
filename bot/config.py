...
#
# 끄면 확정 신호만 보낸다. 정확하지만 **최대 1시간 늦다.**
# 트레이딩뷰 차트는 봉 중간에도 화살표를 그리므로(리페인팅) 차트를 보고 있는
# 사람보다 알림이 한참 늦게 도착하고, 그러면 알림을 쓰는 의미가 없다.
#
# 켜면 트레이딩뷰 화면과 같은 시점에 알림이 간다. 대신 봉이 닫히면서
# 조건이 무효가 될 수 있고, 그 경우 취소 알림이 뒤따른다.
# 잠정 → 확정 또는 잠정 → 취소 로 한 신호당 알림이 2건 발생한다.
