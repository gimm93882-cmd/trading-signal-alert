...
#
# False (현재 설정) — 확정 신호만 보낸다. 봉이 닫히고 조건이 살아남은 것만
# 알리므로 사라지는 신호에 흔들리지 않는다. 대신 트레이딩뷰 화면에 화살표가
# 뜬 시점보다 최대 한 봉(1시간) 늦게 도착한다. 이 지연은 설계상 불가피하다.
#
# True — 트레이딩뷰 화면과 같은 시점에 "잠정"으로 먼저 알린다. 빠르지만
# 봉이 닫히면서 조건이 무효가 될 수 있어 취소 알림이 뒤따르고,
# 한 신호당 알림이 2건(잠정 → 확정 또는 잠정 → 취소) 발생한다.
#
# 잠정 경로의 코드와 테스트는 그대로 남겨둔다. 이 값만 바꾸면 즉시 되살아난다.
