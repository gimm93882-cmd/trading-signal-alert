...
#
# True (현재 설정) — 트레이딩뷰 화면과 같은 시점에 "잠정"으로 먼저 알린다.
# 봉이 닫히기를 기다리지 않으므로 지표에 화살표가 뜨는 즉시(폴링 주기 이내) 도착한다.
# 대신 봉이 닫히면서 조건이 무효가 될 수 있어 취소 알림이 뒤따르고,
# 한 신호당 알림이 2건(잠정 → 확정 또는 잠정 → 취소) 발생한다.
#
# False — 확정 신호만 보낸다. 사라지는 신호에 흔들리지 않지만,
# 1시간봉은 매시 정각에만 닫히므로 알림도 정각 직후에만 나간다.
# 트레이딩뷰 화면에 화살표가 뜬 시점보다 최대 1시간 늦다.
