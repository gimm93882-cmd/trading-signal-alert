...
### 알림은 봉 마감 후에 나갑니다 (확정 전용 · 현재 설정)

**문제.** 확정 신호만 보내면 알림이 트레이딩뷰 화면보다 **최대 1시간 늦는다.**
트레이딩뷰는 봉이 닫히기 전에도 조건이 성립하면 화살표를 그리기 때문이다(리페인팅).

실제로 겪은 사례 (2026-08-31, BTCUSDT):

| 캔들 | 종가 | EMA9 − EMA21 |
|---|---|---|
| 08:00 | 77,633.6 | **+12.25** |
| 09:00 | 77,899.9 | −49.04 ← 확정 |

08:00 봉에서 가격이 78,396 → 77,633 으로 급락하며 **장중에 EMA9 이 EMA21 을
아래로 뚫었다.** 트레이딩뷰에는 그 순간 SELL 화살표가 떴다. 그런데 봉이 닫힐 때
가격이 회복해 +12.25 로 되돌아왔고 화살표는 사라졌다. 다음 봉(09:00)에서 확정됐고
봇은 그 봉이 닫힌 10:00 직후에 보냈다. **화살표를 본 시점과 알림 사이 약 1시간 45분.**

**현재 선택: 확정 전용.** 사라지는 신호에 흔들리지 않도록 봉이 닫히고
조건이 살아남은 것만 알린다. 위와 같은 지연은 이 방식의 대가로 받아들인 것이다.
확정 후 도착까지는 트리거 주기(3분) 안이다.

봉 마감을 기다리지 않는 **잠정 알림 경로도 구현돼 있고 코드·테스트가 그대로 남아 있다.**
`bot/config.py` 의 `PROVISIONAL_ALERTS = True` 로 바꾸면 즉시 되살아난다.
그때의 동작은 아래와 같다.

| 알림 | 제목 예 | 언제 |
|---|---|---|
| 확정 | `🟥 SELL 확정 · BTCUSDT · 77,899.90` | **항상** (현재 설정에서 유일하게 나가는 알림) |
| 잠정 | `⚡ 잠정 SELL · BTCUSDT · 77,728.10 (마감 30분 전)` | `PROVISIONAL_ALERTS = True` 일 때만 |
| 취소 | `✖ 잠정 SELL 취소 · BTCUSDT` | 위와 같음. 잠정이 무효가 됐을 때 |

잠정을 켜면 한 신호당 알림이 2건이 된다 — 잠정 → 확정, 또는 잠정 → 취소.
같은 봉·같은 종류는 반복 발송하지 않는다.

**되돌림으로 봉이 무효가 됐어도 다음 봉이 시작부터 같은 신호라면 취소를 보내지 않는다.**
08:00 봉은 마감 때 되돌아와 무효가 됐지만 09:00 봉은 시가부터 SELL 이었다.
여기서 취소를 보내면 몇 초 뒤 도착하는 잠정 알림과 모순되므로, 조용히 이어간다.

**확정 후 도착까지의 지연 상한은 트리거 주기(현재 3분)와 같다.**
봉 마감 자체를 기다리는 시간은 여기에 포함되지 않는다.
...
그래서 **맥의 launchd 가 3분마다 GitHub 을 깨우는** 구조를 추가했습니다.
...
| `~/Library/LaunchAgents/com.kinampark.trading-signal-trigger.plist` | 3분 주기 등록 |
