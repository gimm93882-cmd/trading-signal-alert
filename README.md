...
**해결.** 봉이 닫히기 전에도 같은 계산을 돌려 조건이 성립하면 즉시 보낸다.
**지표 로직·파라미터는 전혀 바뀌지 않았다.** 언제 들여다보는지만 바뀌었다.

| 알림 | 제목 예 | 의미 |
|---|---|---|
| 잠정 | `⚡ 잠정 SELL · BTCUSDT · 77,728.10 (마감 30분 전)` | 지금 조건 성립. 사라질 수 있음 |
| 확정 | `🟥 SELL 확정 · BTCUSDT · 77,899.90` | 봉이 닫히며 확정됨 |
| 취소 | `✖ 잠정 SELL 취소 · BTCUSDT` | 봉이 닫히며 무효가 됨. 진입 금지 |

한 신호당 알림은 2건이다 — 잠정 → 확정, 또는 잠정 → 취소.
...
끄려면 `bot/config.py` 의 `PROVISIONAL_ALERTS = False`. 끄면 정확하지만 다시 느려진다.

**알림 지연의 상한은 트리거 주기(현재 3분)와 같다.**
